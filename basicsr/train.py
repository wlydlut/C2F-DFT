...
def main():
    ...
    ################patch-cycle
    iters0 = opt['datasets']['train'].get('iters')
    batch_size = opt['datasets']['train'].get('batch_size_per_gpu')
    mini_batch_sizes = opt['datasets']['train'].get('mini_batch_sizes')
    gt_size = opt['datasets']['train'].get('gt_size')
    mini_gt_sizes = opt['datasets']['train'].get('gt_sizes')
    nums = opt['datasets']['train'].get('nums')
    iters = []
    for i in range(len(iters0)):
        iters.extend([iters0[i]] * nums[i] * 3)
    mini_batch_sizes = sum(nums) * mini_batch_sizes
    mini_gt_sizes = sum(nums) * mini_gt_sizes
    #################################
        ...
        while train_data is not None:
            ...
            if logger_j[bs_j]:
                logger.info('\n Updating Patch_Size to {} and Batch_Size to {} \n'.format(mini_gt_size,
                                                                                          mini_batch_size * opt.get('num_gpu')))
            ...
            if mini_batch_size < batch_size:
                ...
                lq = lq[indices]
                gt = gt[indices]
            lq = lq.flatten(start_dim=0, end_dim=1) if lq.ndim == 5 else lq
            gt = gt.flatten(start_dim=0, end_dim=1) if gt.ndim == 5 else gt
            ...
            if opt.get('val') is not None and (current_iter %
                                               opt['val']['val_freq'] == 0 or current_iter == 1):
