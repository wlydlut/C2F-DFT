...
for dataset in datasets:
    ...
    file_path = os.path.join('./results_fine/', dataset)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
        for filename, PSNR_SSIM in zip(img_files, executor.map(proc, img_files)):
            ...
            # with open('results_ssiml12.5w.txt', 'a') as file0:
            #     print('image: %s, PSNR is %.4f and SSIM is %.4f' % (filename, PSNR_SSIM[0], PSNR_SSIM[1]), file=file0)
